...
<body>
	...
	<table class='w-px600'>
		<tr><th class='w-px140'>사번</th>
			<td>${vo.employee_id }</td></tr>
		<tr><th>사원명</th>
			<td>${vo.name}</td></tr>
		<tr><th>이메일</th>
			<td>${vo.email}</td></tr>
		<tr><th>전화번호</th>
			<td>${vo.phone_number}</td></tr>
		<tr><th>부서명</th>
			<td>${vo.department_name}</td></tr>
		<tr><th>업무명</th>
			<td>${vo.job_title}</td></tr>
		<tr><th>급여</th>
			<td>
				<fmt:formatNumber value="${vo.salary}"/>
			</td>
		</tr>
		<tr><th>입사일자</th>
...
<script>
	$('.btn-delete').click(function(){
		if(confirm('사원 [${vo.name}]를 삭제하시겠습니까?')){
