...
//	@Autowired private HrService service;
	...
	public String modify(int id, Model model) {
		// 비지니스 로직
		// 선택한 사원정보를 DB에서 조회한 후
		// 수정화면에 출력될 수 있도록 Model에 attribute로 담는다.
	...
	public String info(int id, Model model) {
		// 비지니스로직
		// DB에서 선택한 사원정보를 조회해온다.
		//service.employee_info(id);
		// 사원정보화면에 출력할 수 있도록 Model에 attribute로 담는다.
		model.addAttribute("vo",service.employee_info(id));
		// 응답화면연결
